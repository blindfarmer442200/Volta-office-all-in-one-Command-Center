...
Memory is inserted only after the input gate defers a request to an LLM.
Blocked and deterministic-answerable requests never read the memory store. The
model response still passes through output scanning before return.
"""
...
class BellaHarness:
    ...
    def memory_enabled(self) -> bool:
        # Empty memory preserves the original prompt, so enabling the boundary
        # by default does not alter existing deployments.
    ...
    def _memory_content_is_safe(self, content: str) -> bool:
        """Reject memory text the deterministic input gate classifies BLOCK."""
    ...
    def handle(self, request_text: str, *, mode: str = "default") -> HarnessResult:
        ...
        if self.memory_enabled:
            try:
                ...
                # Fail-open never passes partial memory. It uses the untouched
                # request with no memory context.
                prompt = request_text
